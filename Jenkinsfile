    ...
    stages{
        ...
        stage('Deploy to k8s'){
            ...
            sshagent(['sshk8s']) {
               ...
               sh "ssh -o StrictHostKeyChecking=no root@10.182.0.33 /usr/bin/kubectl apply -f /root/cicd/services.yml"
               sh "ssh -o StrictHostKeyChecking=no root@10.182.0.33  /usr/bin/kubectl apply -f  /root/cicd/node-app-pod.yml"
                }
            }
        }
